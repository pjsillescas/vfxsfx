	...
	else
	{
		...
		else
		{
			...
			else
			{
				...
				colorTexureShader.init("colorTexture2Mix");
				initGL();
...
void initGL()
{
	...
	UniformIDMixTexture = glGetUniformLocation(colorTexureShader.getID(), "mixtextures");

	//Create Texture
	SDL_Surface* tempSurface;
	tempSurface = IMG_Load("Assets/textures/batmanlogo.png");
	...
	//Create Texture2
	tempSurface = IMG_Load("Assets/textures/jockerlogo.png");
	...
	glTexImage2D(GL_TEXTURE_2D, 0, Mode, tempSurface->w, tempSurface->h, 0, Mode, GL_UNSIGNED_BYTE, tempSurface->pixels);
	glGenerateMipmap(GL_TEXTURE_2D);
	// Same TexParameter as other Texture
	glBindTexture(GL_TEXTURE_2D, 0);
...
void render()
{
	...
	glBindTexture(GL_TEXTURE_2D, texture);
	glUniform1i(glGetUniformLocation(colorTexureShader.getID(), "textura"), 0);
	glActiveTexture(GL_TEXTURE1);
	glBindTexture(GL_TEXTURE_2D, texture2);
	glUniform1i(glGetUniformLocation(colorTexureShader.getID(), "textura2"), 1);
	//Mix textures value
	glUniform1f(UniformIDMixTexture, MixValue);
	//Set VAO
	glBindVertexArray(gVAO);
