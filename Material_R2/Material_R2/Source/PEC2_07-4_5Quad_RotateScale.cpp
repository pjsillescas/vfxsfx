...
float Xpos[NQUADS];
float Ypos[NQUADS];
float Velocity[NQUADS];
float Scale[NQUADS];

void randomQuadsAgain()
{
	// Init X- Y positions
	for (int i = 0; i < NQUADS; i++) {
		Xpos[i] = ((rand() % 2000) - 1000) / 1000.0f;
		Ypos[i] = ((rand() % 2000) - 1000) / 1000.0f;
		Velocity[i] = static_cast<float>(2 * (rand() % 25) + 1);
		Scale[i] = ((rand() % 100) + 1) / 100.0f;
	}
}
...
void render()
{
	...
	//Do Matrix movements
	for (int i = 0; i < NQUADS; i++) {
		degrees = (SDL_GetTicks() / Velocity[i]);
		transformMatrix = glm::mat4();
		transformMatrix = glm::translate(transformMatrix, glm::vec3(Xpos[i], Ypos[i], 0));
		transformMatrix = glm::rotate(transformMatrix, glm::radians(degrees), glm::vec3(0.0, 0.0, 1.0));
		transformMatrix = glm::scale(transformMatrix, glm::vec3(Scale[i], Scale[i], 0.0));
...
int main(int argc, char* args[])
{
	...
	{
		printf("* Press SPACE for random again *");
		//Main loop flag
		bool quit = false;
